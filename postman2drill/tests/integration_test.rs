...
fn run_converter(args: &[&str]) -> (String, String) {
    ...
    let output = Command::new(bin)
        .args(args)
        .output()
        .expect("failed to execute postman2drill");
    (
...
fn test_converter_outputs_expected_yaml() {
    ...
    let (_stdout, stderr) = run_converter(&[
    ...
    assert!(
        stderr.contains("Drill benchmark written to"),
...
fn test_converter_warnings_json() {
    ...
    let (_stdout, _stderr) = run_converter(&[
        collection.to_str().unwrap(),
        "-o",
        out.to_str().unwrap(),
        "-w",
        warnings.to_str().unwrap(),
        "-f",
        "json",
    ]);

...
pub enum DrillBody {
    Template(String),
    Binary { hex: String },
    BinaryFile { file: String },
    UrlEncoded(HashMap<String, String>),
    FormData(Vec<FormDataPart>),
    GraphQL { query: String, variables: Option<HashMap<String, String>> },
}
...
impl Serialize for DrillBody {
        ...
        S: serde::Serializer,
    {
        let value = match self {
            ...
            DrillBody::Binary { hex } => serde_yaml::to_value(serde_json::json!({ "hex": hex })).unwrap(),
            DrillBody::BinaryFile { file } => serde_yaml::to_value(serde_json::json!({ "file": file })).unwrap(),
            DrillBody::UrlEncoded(map) => {
                serde_yaml::Value::Mapping(serde_yaml::Mapping::from_iter(vec![(
                    serde_yaml::Value::String("urlencoded".to_string()),
                    serde_yaml::to_value(map).unwrap(),
                )]))
            }
            DrillBody::FormData(parts) => {
                serde_yaml::Value::Mapping(serde_yaml::Mapping::from_iter(vec![(
                    serde_yaml::Value::String("formdata".to_string()),
                    serde_yaml::to_value(parts).unwrap(),
                )]))
            }
            DrillBody::GraphQL { query, variables } => {
                ...
                serde_yaml::Value::Mapping(serde_yaml::Mapping::from_iter(vec![(
                    serde_yaml::Value::String("graphql".to_string()),
                    serde_yaml::Value::Mapping(graphql_map),
                )]))
            }
        };
...
pub enum WithItemsFromCsv {
    Simple(String),
    Detailed { file_name: String, #[serde(skip_serializing_if = "Option::is_none")] quote_char: Option<String> },
}
...
pub enum AssertConfig {
    ...
    Status { #[serde(rename = "type")] _type: String, value: serde_yaml::Value },
    Header { #[serde(rename = "type")] _type: String, key: String, value: String },
    JsonPath { #[serde(rename = "type")] _type: String, key: String, value: String },
    Duration { #[serde(rename = "type")] _type: String, value: u64, #[serde(skip_serializing_if = "Option::is_none")] operator: Option<String> },
}

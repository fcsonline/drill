...
use crate::model::collection::{Item as CollectionItem, RequestItem as CollectionRequestItem};
use crate::model::drill::{DrillConfigInput, RequestItem as DrillRequestItem};
...
impl Converter {
  ...
  pub fn convert(&mut self, collection: Collection, environment: Option<Environment>, config_input: Option<DrillConfigInput>) -> Result<DrillBenchmark> {
    ...
    if !collection.event.is_empty() {
      ...
      if !setup_items.is_empty() || !iteration_start_items.is_empty() {
        lifecycle = Some(crate::model::Lifecycle {
          setup: if setup_items.is_empty() {
            None
          } else {
            Some(setup_items)
          },
          iteration_start: if iteration_start_items.is_empty() {
            None
          } else {
            Some(iteration_start_items)
          },
          teardown: None,
          iteration_stop: None,
        });
    ...
    let (base, concurrency, iterations, rampup, results, load_shape) = config_input.map(|c| (c.base, c.concurrency, c.iterations, c.rampup, c.results, c.load_shape)).unwrap_or_default();
    ...
    Ok(DrillBenchmark {
      ...
      iterations,
      rampup,
      vars: if self.variable_ctx.vars.is_empty() {
        None
      } else {
        Some(self.variable_ctx.vars.clone())
      },
      lifecycle,
      results,
      load_shape,
  ...
  pub fn convert_item(&mut self, item: CollectionItem, location: &str) -> Result<Vec<PlanItem>> {
    match item {
      CollectionItem::Request(req) => self.convert_request(req, location),
  ...
  fn build_request(&mut self, name: String, request: crate::model::Request, location: &str) -> Result<DrillRequestItem> {
    let drill_request = crate::convert::request::convert_request(request, &self.variable_ctx, self.collection_auth.as_ref(), &mut self.warnings, location)?;

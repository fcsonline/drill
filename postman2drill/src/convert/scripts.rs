...
use crate::model::drill::{PlanItem, AssignItem, AssignConfig, SaveItem, SaveConfig, AssertItem, AssertConfig};
use regex::Regex;
use anyhow::Result;
...
fn try_convert_pre_request(
    ...
    if let Some(caps) = re.captures(line) {
        ...
        return Ok(Some(PlanItem::Assign(AssignItem {
            ...
            assign: AssignConfig { key: key.to_string(), value },
            weight: None,
        })));
    }
...
fn try_convert_test_assertion(
    ...
    if let Some(caps) = Regex::new(r#"pm\.response\.to\.have\.status\((\d+)\)"#).unwrap().captures(line) {
        return Ok(Some(PlanItem::Assert(AssertItem {
            ...
            assert: AssertConfig::Status { _type: "status".to_string(), value: serde_yaml::Value::Number(caps[1].parse().unwrap()) },
            weight: None,
        })));
    }
    ...
    if let Some(caps) = Regex::new(r#"pm\.response\.to\.have\.status\(\[([^\]]+)\]\)"#).unwrap().captures(line) {
        let codes: Vec<serde_yaml::Value> = caps[1].split(',')
            .map(|s| serde_yaml::Value::Number(s.trim().parse().unwrap()))
            .collect();
        return Ok(Some(PlanItem::Assert(AssertItem {
            name: Some("Response status".to_string()),
            assert: AssertConfig::Status { _type: "status".to_string(), value: serde_yaml::Value::Sequence(codes) },
            weight: None,
        })));
    }
    ...
    if let Some(caps) = Regex::new(r#"pm\.response\.to\.have\.header\(["']([^"']+)["']\s*,\s*["']([^"']+)["']"#).unwrap().captures(line) {
        return Ok(Some(PlanItem::Assert(AssertItem {
            ...
            assert: AssertConfig::Header { _type: "header".to_string(), key: caps[1].to_string(), value: caps[2].to_string() },
            weight: None,
        })));
    }
    ...
    if let Some(caps) = Regex::new(r#"pm\.expect\(pm\.response\.jsonPath\(["']([^"']+)["']\)\)\.to\.eql\((.+)\)"#).unwrap().captures(line) {
        ...
        return Ok(Some(PlanItem::Assert(AssertItem {
            ...
            assert: AssertConfig::JsonPath { _type: "jsonpath".to_string(), key: caps[1].to_string(), value },
            weight: None,
        })));
    }

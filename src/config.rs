...
impl Config {
    pub fn new(path: &str, relaxed_interpolations: bool, no_check_certificate: bool, quiet: bool, nanosec: bool, timeout: u64, verbose: bool) -> Result<Config, io::Error> {
        let config_file = reader::read_file(path)?;

        let config_docs = match YamlLoader::load_from_str(config_file.as_str()) {
            Ok(docs) => docs,
        ...
        if concurrency > iterations {
            return Err(io::Error::new(io::ErrorKind::InvalidInput, "The concurrency can not be higher than the number of iterations"));
        }
...
fn read_str_configuration(config_doc: &Yaml, interpolator: &interpolator::Interpolator, name: &str, default: &str) -> Result<String, io::Error> {
    match config_doc[name].as_str() {
        Some(value) => {
            if value.contains('{') {
                Ok(interpolator.resolve(value, true)?)
            } else {
                Ok(value.to_owned())
            }
        }
...
fn read_i64_configuration(config_doc: &Yaml, interpolator: &interpolator::Interpolator, name: &str, default: i64) -> Result<i64, io::Error> {
    ...
    match value {
        Some(value) => {
            if value < 0 {
                eprintln!("Invalid negative {name} value!");

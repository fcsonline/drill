...
impl Request {
  ...
  pub fn new(item: &Yaml, with_item: Option<Yaml>, index: Option<u32>) -> Request {
    ...
    let body = if body_verbs.contains(&method.as_str()) {
      if let Some(body) = item["request"]["body"].as_str() {
        Some(Body::Template(body.to_string()))
      } else if let Some(file_path) = item["request"]["body"]["file"].as_str() {
        let mut file = File::open(file_path).expect("Unable to open file");
        let mut buffer = Vec::new();
        file.read_to_end(&mut buffer).expect("Unable to read file");
        Some(Body::Binary(buffer))
      } else if let Some(hex_str) = item["request"]["body"]["hex"].as_str() {

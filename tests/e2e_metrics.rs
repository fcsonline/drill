...
"#,
    ...
    let output = Command::new(common::drill_bin())
        .arg("--benchmark")
        .arg(benchmark)
        .arg("--stats")
        .output()
        .expect("drill binary should run");
    ...
    let line = csv_content
        .lines()
        .find(|l| l.starts_with("Get root"))
        .expect("Get root row should exist");
    let total_download: u64 = line
        .split(',')
        .nth(26)
        .expect("total download column should exist")
        .parse()
        .expect("total download should be a number");

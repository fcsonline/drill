...
impl MockApi {
    ...
    pub async fn start() -> Self {
        ...
        Mock::given(method("POST"))
            .and(path("/echo"))
            .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({"received": "{{ body }}"})))
            .mount(&server)
            .await;

        Mock::given(method("GET"))
            .and(path("/delay"))
            .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({"status": "delayed"})))
            .mount(&server)
            .await;
...
pub fn drill_bin() -> PathBuf {
    std::env::var("CARGO_BIN_EXE_drill")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("target/debug/drill"))
}
